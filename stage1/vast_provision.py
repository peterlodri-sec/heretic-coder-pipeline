...
def provision(vast, label: str = LABEL):
    ...
    match existing.get("actual_status"):
        case "running":
            return existing
        case _:
            # Stopped labeled instance: start it. Do NOT swallow a start failure
            # and fall through to renting — that masks a persistent API error as
            # "instance is gone" and orphans a second, billed instance.
            # Terminated instances drop out of show_instances entirely, so
            # reaching here means the instance is still ours to restart.
