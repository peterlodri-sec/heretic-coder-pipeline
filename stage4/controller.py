#!/usr/bin/env python3
# stage4/controller.py — RFT / rejection-sampling self-improvement loop. Runs
# BETWEEN SFT (stage2) and the terminal RLVR stage (stage5). 1x H200 is feasible.
import argparse
import os
import sys
...
STAGE4_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.dirname(STAGE4_DIR)
SHARED_DIR = os.path.join(REPO_ROOT, "shared")
STAGE2_DIR = os.path.join(REPO_ROOT, "stage2")  # RFT reuses stage2's sft_train
REMOTE_PARENT = "/root"
REMOTE_ROOT = "/root/stage4"
REMOTE_STATUS_PATH = f"{REMOTE_ROOT}/remote/status.json"
REMOTE_LOG_PATH = f"{REMOTE_ROOT}/remote/rft_run.log"
POLL_INTERVAL_SECONDS = 300
SETUP_TIMEOUT_SECONDS = 1800
PROVISION_LABEL = "heretic-rft"
PROVISION_QUERY = "gpu_name=H200 disk_space>=400 rentable=true"  # single H200
...
def deploy_and_launch(instance: dict, model: str, rounds: int, num_candidates: int,
    ...
    ssh_utils.scp_to(host, port, STAGE2_DIR, REMOTE_PARENT, recursive=True)  # sft_train
    ssh_utils.scp_to(host, port, STAGE4_DIR, REMOTE_PARENT, recursive=True)
    ...
    try:
        ...
        try:
            ssh_utils.scp_from(host, port, REMOTE_LOG_PATH,
                               os.path.join(STAGE4_DIR, "rft_run.log"))
