...
def test_new_status_defaults():
    ...
    for f in ("round", "num_rounds", "candidates_generated", "candidates_passing",
              "train_loss", "refusal_rate", "bfcl_accuracy", "humaneval_delta",
...
def test_enum_round_trip():
    s = Status.new("1")
    s.stage = Stage.VERIFYING
    s.verdict = Verdict.PASS
    s.round = 2
    loaded = Status.from_json(s.to_json())
    assert loaded.stage is Stage.VERIFYING and loaded.verdict is Verdict.PASS
