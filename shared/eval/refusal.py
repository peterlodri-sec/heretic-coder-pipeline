...
def refusal_rate(model_path, prompts, family: str = "gpt_oss") -> float:
    ...
    try:
        ...
        # gpt-oss is harmony-channelled: score refusal on the `final` channel
        # only, never the analysis CoT (extract_final is a no-op on qwen text).
        ...
        # Free this model before the next eval loads its own (2xH200 cannot hold
        # two 120B models at once). Drop local refs first, then reclaim memory.
        del model, tokenizer
        free_model()
